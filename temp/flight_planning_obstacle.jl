...
# ╔═╡ a9b96a80-b19b-4b1a-972e-54dd3ed6c9c8
function min_prime(y)
	if y[3] > 1.0 - h_ref
		return 0.0
	else
		return 1.0
...
function F_at(Integrand, y, ydot, B, Bdot)
	  return (ydot - w(y, Integrand.scaling_wind))'*(Bdot - w_prime(y, Integrand.scaling_wind)*B) + Integrand.scaling_penalty * min(0.0, (y[3] - 1.0 + h_ref))*B[3]
...
function F_prime_at(Integrand,y,ydot,B1,B1dot,B2,B2dot)
	...
	*B2) + Integrand.scaling_penalty*min_prime(y)*B1[3]*B2[3]
end
...
ws = [w(p, integrand.scaling_wind) for p in y_0]
...
wireframe!(ax, sx, sy, sz, color = RGBA(0.5,0.5,0.7,0.1); transparency=true)
	...
	scatterlines!(ax, sx[:,38], sy[:,38], sz[:,38]; markersize =2, color=:black, linewidth=2)

	println(1-cos(v[38])) #das ist mein h_ref
	...
	arrows!(ax, π1.(y_0), π2.(y_0), π3.(y_0), π1.(ws), π2.(ws), π3.(ws); color=:green, linewidth=0.01, arrowsize=Vec3f(0.03, 0.03, 0.05), transparency=true, lengthscale=0.5)
