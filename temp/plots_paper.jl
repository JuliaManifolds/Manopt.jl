...
# ╔═╡ 42689232-5bf5-11f0-0e6b-ad11c8ba357f
begin
...
# ╔═╡ eeb7fdd9-da62-4ee9-8e41-3d4dc69dd023
begin
	geodesic_final_tab = readdlm("geodesic_force_final.csv", ',')
	geodesic_final = [geodesic_final_tab[:, i] for i in 1:size(geodesic_final_tab, 2)]
	
	geodesic_start_tab = readdlm("geodesic_force_start.csv", ',')
	geodesic_start = [geodesic_start_tab[:, i] for i in 1:size(geodesic_start_tab, 2)]
	
	force_final_tab = readdlm("geodesic_forcefield_final.csv", ',')
	force_final = [force_final_tab[:, i] for i in 1:size(force_final_tab, 2)]
	
	force_start_tab = readdlm("geodesic_forcefield_start.csv", ',')
	force_start = [force_start_tab[:, i] for i in 1:size(force_start_tab, 2)]
	
	normen_deltax = readdlm("norm_newton_direction_geodesic_force.csv", ',')
end;

# ╔═╡ bc76d741-cfdd-4e28-8fbc-aa561e27a95b
begin
	f = Figure(;)
	
    row, col = fldmod1(1, 2)
	
	Axis(f[row, col], yscale = log10, title = string("Norms of the Newton directions (semilogarithmic)"), xminorgridvisible = true, xticks = (1:length(normen_deltax)), xlabel = "Iteration", ylabel = "‖δx‖")
    scatterlines!(normen_deltax[1:end], color = :blue)
	f

	#save("/Users/bt308990/Documents/Publications/dmv/gfx_pdf/convergence_plot.pdf", f)
	
end

# ╔═╡ afba46fa-5b8d-4e39-82f8-86e8602afc62
begin
n = 25
...
wireframe!(ax, sx, sy, sz, color = RGBA(0.5,0.5,0.7,0.1); transparency=true)
	...
	arrows!(ax, π1.(geodesic_final[2:end-1]), π2.(geodesic_final[2:end-1]), π3.(geodesic_final[2:end-1]), π1.(force_final), π2.(force_final), π3.(force_final); color=:green, linewidth=0.01, arrowsize=Vec3f(0.03, 0.03, 0.05), transparency=true, lengthscale=0.07)
...
manifest_format = "2.0"
project_hash = "0bd4609c415baae6ed944a0e8f51a98111cc3571"
