...
h = RHess(M,A,p)
g = RGrad(M,A)
x_opt2 = trust_regions(M, cost, g, h, x; max_trust_region_radius=8.0, evaluation=MutatingEvaluation())

@btime trust_regions($M, $cost, $g, $h, x2; max_trust_region_radius=8.0, evaluation=$(MutatingEvaluation()))  setup = (x2 = deepcopy($x))

x3 = deepcopy(x)
trust_regions!(M, cost, g, h, x3; max_trust_region_radius=8.0, evaluation=MutatingEvaluation())

@btime trust_regions!($M, $cost, $g, $h, x3; max_trust_region_radius=8.0, evaluation=$(MutatingEvaluation()))  setup = (x3 = deepcopy($x))
