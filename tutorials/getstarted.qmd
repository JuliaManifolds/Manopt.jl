...
See ``[here](@ref Manopt.RecordActionFactory(s::AbstractManoptSolverState, ::Symbol))``{=commonmark} for the list of available symbols.
...
The default stopping criterion for [`gradient_descent`](@ref) is, to either stop when the gradient is small (`<1e-9`) or a max number of iterations is reached (as a fallback).
...
a little more expensive [`ArmijoLinesearch`](@ref) than the default [stepsize](../commons/stepsizes.md) rule used on the Sphere.
...
But since every is a distance and its ``[proximal map](@extref ManifoldDiff :jl:function:`ManifoldDiff.prox_distance`)``{=commonmark} is available in ``[ManifoldDiff.jl](@extref ManifoldDiff :std:doc:`index`)``{=commonmark}, we can use the
