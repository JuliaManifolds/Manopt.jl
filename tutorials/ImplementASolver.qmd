...
`f(M,p) = ...`. More generally, it might for example store a gradient function or the Hessian
...
for example ``[`debug=` for debug](@ref DebugSection)``{=commonmark}, ``[`record=`](@ref RecordSection)``{=commonmark} keywords for debug and recording
within solver states or ``[`count=` and `cache`](@ref ObjectiveSection)``{=commonmark} keywords for the objective.
...
This could be considered as an interim step to the high-level interface:
If objective, a [`ManifoldCostObjective`](@ref) is already initialized,
the high level interface consists of the steps
