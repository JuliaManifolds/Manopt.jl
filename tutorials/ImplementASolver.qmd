---
title: "How to implement your own solver"
...
Usually the problem's variable is called `mp`.
...
Parametrizing the state avoids that we have abstract typed fields.
...
There are basically only four methods we need to implement for our solver

- `initialize_solver!(mp, rws)` which initializes the solver before the first iteration
- `step_solver!(mp, rws, k)` which implements the `k`th iteration, where `k` is given to you as the third parameter
- `get_iterate(rws)` which accesses the iterate from other places in the solver
- `get_solver_result(rws)` returning the solver's final (best) point we reached.
...
The state we defined before uses the field names common in `Manopt.jl`, that is the
[`StoppingCriterion`](@ref) is stored in `stop` and the iterate in `p`.
A default [`get_stopping_criterion`](@ref) relies on the stopping criterion being in `stop`. It has to be overwritten for solvers not following this naming scheme.
Furthermore the function `stop_solver!(mp, rws, k)` used to determine whether or not to stop after the `k`th iteration relies on the stopping criterion being stored at that place.

We recommend to follow the general scheme with the `stop` field.
`Manopt.jl` already provides a library of available [stopping criteria](../commons/stopping_criteria.md) you can use.
...
function initialize_solver!(mp::AbstractManoptProblem, rws::RandomWalkState)
    ...
    copyto!(M, rws.q, rws.p) # Set best q to p^{(0)}
    return rws
end
...
(and also the objective) has access functions for its elements,
the one we need is [`get_cost`](@ref).

```{julia}
#| output: false
function step_solver!(mp::AbstractManoptProblem, rws::RandomWalkState, k)
    M = get_manifold(mp) # for ease of use get the manifold from the problem
    X = rand(M; vector_at = rws.p)     # generate a direction
    X .*= rws.σ / norm(M, rws.p, X)
    # Walk
    retract!(M, rws.p, rws.p, X, rws.retraction_method)
    # is the new point better? Then store it
    if get_cost(mp, rws.p) < get_cost(mp, rws.q)
        copyto!(M, rws.q, rws.p)
    end
    return rws
end
```
...
this solver to allow for [caching](CountAndCache.md).
...
the result in general; we could also just look at `s.q`, but the function `get_solver_result` is
...
for example [`debug = `](../commons/debugs.md), [`record = `](../commons/records.md) keywords for debug output and recording
within solver states or `count=` and `cache` keywords for the objective.

We can introduce these here as well with just a few lines of code. There are usually two steps.
We further need three more functions from `Manopt.jl`, two of which are internal
...
This could be considered as an interim step to the high-level interface: if the objective, a [`ManifoldCostObjective`](@ref), is already initialized, the high-level interface consists of the steps

1. possibly decorate the objective
2. generate the problem
3. generate and possibly decorate the state
4. call the solver
...
function random_walk_algorithm!(
    ...
    s = RandomWalkState(
        M, p;
        σ = σ, retraction_method = retraction_method, stopping_criterion = stopping_criterion,
    )
...
For the case that you set `return_state=true` the solver should return a summary of the run.
...
Compared to Julia's own `show(io::IO, state)` method, the status summary aims to be
in prose text.
...
# the `:short` context returns `repr`, so the state needs a compact `show` of its parameters
function Base.show(io::IO, rws::RandomWalkState)
    return print(
        io,
        "RandomWalkState(; σ = $(rws.σ), retraction_method = $(rws.retraction_method), stopping_criterion = $(rws.stop))",
    )
end
...
```{julia}
rws = random_walk_algorithm!(M, f; return_state = true)
```
...
One feature we did not cover is that most algorithms allow for in-place and allocating functions, as soon as they work on more than just the cost, for example use gradients, proximal maps or Hessians.
