...
To use more advanced stopping criteria and step sizes we first need an[`inner`](@extref `ManifoldsBase.inner-Tuple{AbstractManifold, Any, Any, Any}`)`(M, p, X)`.
...
To initialize tangent vector memory, the function [`zero_vector`](@extref `ManifoldsBase.zero_vector-Tuple{AbstractManifold, Any}`)`(M,p)` is called. Similarly,
