...
When you have used a few solvers from [`Manopt.jl`](../index.md) for example like in the opening
...
``[🚀 Get Started with `Manifolds.jl`](@extref Manifolds :std:doc:`tutorials/getstarted`)``{=commonmark},
you might come across the point that you want to
``[implement a manifold](@extref :doc:`tutorials/implement-a-manifold`)``{=commonmark}
yourself and use it within [`Manopt.jl`](../index.md).
...
We would like to compute a mean similar to [🏔️ Get started with Manopt.jl](getstarted.md).
For a given set of points $q_1,\ldots,q_n$ we want to compute [Karcher:1977](@cite)
...
pts = [ [zeros(d)..., M.radius] .+ 0.5 .* ([rand(d)..., 0.5] .- 0.5) for _ in 1:N]
...
to define the cost function, as well as the logarithmic map to define the gradient.
For both, we here use the “lazy” approach of using the ``[Sphere](@extref `Manifolds.Sphere`)``{=commonmark} as a fallback.
...
function log!(M::ScaledSphere, X, p, q)
    log!(Sphere(M.dimension), X, p ./ M.radius, q ./ M.radius)
    X .*= M.radius
    return X
end
...
grad_f(M, q) = sum(- 2 * log(M, q, p) for p in pts)
```
...
for our case that is ``[The gradient descent's Technical Details](@ref sec-gradient-descent-technical-details)``{=commonmark},
...
To be precise, we have to implement the ``[in-place variant](@extref ManifoldsBase `inplace-and-noninplace`)``{=commonmark} ``[`retract_project!`](@extref `ManifoldsBase.retract_project!-Tuple{AbstractManifold, Any, Any, Any}`)``{=commonmark}
...
We start with a certain point and evaluate its cost
```{julia}
p0 = [r, zeros(d)...]
f(M, p0)
```
...
To use more advanced stopping criteria and step sizes we first need an ``[`inner`](@extref `ManifoldsBase.inner-Tuple{AbstractManifold, Any, Any, Any}`)``{=commonmark}`(M, p, X, Y)`.
...
# set the maximal allowed step size to injectivity radius.
...
```{julia}
gradient_descent(M, f, grad_f, p0; debug = [:Iteration, :Cost, " | ", :Stepsize, " | ", :GradientNorm, 25, "\n", :Stop]);
```

see [How to print debug output](HowToDebug.md) for more details.
