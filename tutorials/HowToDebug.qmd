...
from “within” the solver. Therefore, passing a function to `debug=` is the way to
...
or analogously [`Manopt.DebugCallback`](@ref)`(callback; simple=true)` and use that in arrays or [`DebugGroup`](@ref)s as above.
The `callback2` can also be part of a whole `debug = [...]` array,
similarly within a dictionary to add callbacks only to the end of an algorithm (`:Stop => callback2`) or to `:BeforeIteration` as illustrated above.
