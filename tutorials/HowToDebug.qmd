...
example that includes a sub solver, to also consider their debug capabilities.
...
* the last number in the array is used with [`DebugEvery`](@ref) to print the debug only every $k$th iteration.
...
While in the last step, we specified what to print, this can be extended to even specify _when_ to print it. Currently the following four “places” are available, ordered by when they appear
in an algorithm run.

* `:Start` to print something at the start of the algorithm. At this place all other (the following) places are “reset”: `:BeforeIteration` and `:Iteration` are triggered with an iteration number `0`, so they may already print, while `:Stop` is triggered with `-1`, which resets it without printing
...
the last code block `[:Iteration, :Cost, " | ", (:ϵ,"ϵ: %.8f"), 25,]` is added to this entry.
* `:Stop` to print something when the algorithm stops. In the example, the `:Stop` symbol adds the [`DebugStoppingCriterion`](@ref) to this place.
...
Sub solvers have a `sub_kwargs` keyword, such that you can pass keywords to the sub solver as well. This works well if you do not plan to change the subsolver. If you do you can wrap your own `sub_state=` argument in a [`decorate_state!`](@ref) and pass a `debug=` keyword to this function call.
...
A main problem now is, that this debug is issued every sub solver call or initialization, as the following print of just a `.` per sub solver test/call illustrates
...
The different lengths of the dotted lines come from the fact that ---at least in the beginning--- the subsolver performs a few steps and each sub solver's step prints a dot.

For this issue, there is the next symbol (similar to the `:Stop`) to indicate that a debug set is a sub solver set `:WhenActive`, which introduces a [`DebugWhenActive`](@ref) that is only activated when the outer debug is actually active, or in other words [`DebugEvery`](@ref) is active itself.
Furthermore, we want to print the iteration number _before_ printing the sub solver's steps, so we put this into a `Pair`, but we can leave the remaining ones as single
entries.
...
where we now see that the sub solver requires only one or two steps. Since the iteration number is printed in `:BeforeIteration`, that is _before_ the step, the sub solver run of an iteration is shown _after_ its iteration number and _before_ the cost, which is printed in `:Iteration` after the step.

## Advanced debug output

There are two more advanced variants that can be used. The first is a tuple of a symbol and a string, where the string is used as the format print, that most [`DebugAction`](@ref)s have. The second is, to directly provide a `DebugAction`.
...
You can also write your own [`DebugAction`](@ref) functor, where the function to implement has the same signature as the `step` function, that is an [`AbstractManoptProblem`](@ref), an [`AbstractManoptSolverState`](@ref), as well as the current iteration `k`. For example, a simplified version of the already mentioned [`DebugDivider`](@ref) could look like
