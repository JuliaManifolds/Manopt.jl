...
where we now see that the sub solver requires only a single step. Since the iteration number is printed in `:BeforeIteration`, that is _before_ the step, the sub solver run of an iteration is shown _after_ its iteration number and _before_ the cost, which is printed in `:Iteration` after the step.

## Advanced debug output

Both variants introduced above, a tuple of a symbol and a format string and a [`DebugAction`](@ref)
provided directly, can also be combined in one `debug=` array.
