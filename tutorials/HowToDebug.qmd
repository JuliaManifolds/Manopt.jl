...
where we now see that the sub solver requires only one or two steps. Note that since debug of an iteration is happening _after_ a step, we see the sub solver run _before_ the debug for an iteration number.
