...
You can also write your own [`DebugAction`](@ref) functor, where the function to implement has the same signature as [`step_solver!`](@ref), that is an [`AbstractManoptProblem`](@ref), an [`AbstractManoptSolverState`](@ref), as well as the current iteration `k`. For example, a simplified version of the already mentioned [`DebugDivider`](@ref) could look like
