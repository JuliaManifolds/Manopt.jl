...
From the returned state, we see that the [`GradientDescentState`](@ref) are encapsulated (decorated) within a [`RecordSolverState`](@ref).
...
This can be also done by using a the high level interface [`get_record`](@ref)
...
We first generate the problem and the state, to also illustrate the low-level works when not using the high-level interface [`gradient_descent`](@ref).
...
R4 = gradient_descent(
    M, f4, grad_f, data[1];
    record=[RecordCount(),], return_state=true,
)
```
