...
For the high level interfaces of the solvers, like [`gradient_descent`](@ref) we have to set `return_state` to `true` to obtain the whole [solver state](../plans/state.md) and not only the resulting minimizer.
...
Here, the symbol `:Cost` is mapped to using the [`RecordCost`](@ref) action. The same holds for `:Iteration` obviously records the current iteration number `i`.
