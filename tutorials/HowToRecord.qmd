...
* record within a sub solver;
* define an own `RecordAction` to perform individual recordings.

Several predefined recordings exist, for example [`RecordCost`](@ref) or [`RecordGradient`](@ref), if the problem the solver uses provides a gradient.
For fields of the `State` the recording can also be done using [`RecordEntry`](@ref).
For other recordings, for example more advanced computations before storing a value, an own `RecordAction` can be defined.

We illustrate these using the gradient descent from the [🏔️ Get started with Manopt.jl](getstarted.md) tutorial.

Here the focus is put on ways to investigate the behavior during iterations by using recording techniques.
...
For the high-level interfaces of the solvers, like [`gradient_descent`](@ref) we have to set `return_state` to `true` to obtain the whole [solver state](../base/state.md) and not only the resulting minimizer.

Then we can easily use the `record=` option to add recorded values. This keyword accepts [`RecordAction`](@ref)s
as well as several symbols as shortcuts, for example `:Cost` to record the cost, or if your state has a field `f`, `:f` would record that entry.
An overview of the symbols that can be used is given ``[here](@ref Manopt.RecordActionFactory(s::AbstractManoptSolverState, ::Symbol))``{=commonmark}.
...
From the returned state, we see that the [`GradientDescentState`](@ref) is encapsulated (decorated) within a [`RecordSolverState`](@ref).

For such a state, one can attach different recorders to some operations, currently to `:Start`, `:Stop`, and `:Iteration`, where `:Iteration` is the default when using the `record=` keyword with a [`RecordAction`](@ref) or a `Symbol` as we just did.
We can access all values recorded during the iterations by calling `get_record(R, :Iteration)` or since this is the default even shorter

```{julia}
get_record(R)
```

To record more than one value, you can pass an array of a mix of symbols and [`RecordAction`](@ref)s which formally introduces [`RecordGroup`](@ref). Such a group records a tuple of values in every iteration:

```{julia}
R2 = gradient_descent(M, f, grad_f, data[1]; record = [:Iteration, :Cost], return_state = true)
```

Here, the symbol `:Cost` is mapped to using the [`RecordCost`](@ref) action. The same holds for `:Iteration`, which obviously records the current iteration number `k`.
...
This can also be done by using the high-level interface [`get_record`](@ref)
...
```{julia}
get_record(R2, :Iteration, (:Cost, :Iteration))
```
...
We first generate the problem and the state, to also illustrate how the low-level works when not using the high-level interface [`gradient_descent`](@ref).
...
We now first build a  [`RecordGroup`](@ref) to group the three entries we want to record per iteration. We then put this into a [`RecordEvery`](@ref) to only record this every sixth iteration
...
We now combine both into the [`RecordSolverState`](@ref) decorator. It acts completely the same as any [`AbstractManoptSolverState`](@ref) but records something in every iteration additionally. This is stored in a dictionary of [`RecordAction`](@ref)s, where `:Iteration` is the key for the group `rI` (recording only every sixth iteration) and `:Stop` the key for `sI`, which is executed at stop.

Note that the keyword `record=` in the high-level interface [`gradient_descent`](@ref) only would fill the `:Iteration` symbol of said dictionary, but we could also pass pairs like in the following,
...
## Recording from a sub solver

One can also record from a sub solver. For that we need a problem that actually requires a sub solver. We take the constraint example from the
...
We directly start with recording the sub solver's iteration.
We can specify what to record in the subsolver using the `sub_kwargs`
keyword argument with a `Symbol => value` pair. Here we specify to record the iteration and the cost in every sub solver's step.
...
Then the first entry of the record contains the iteration `k`, the (main solver's) cost, and the third entry is the recording of the subsolver.
...
Finally, instead of recording iterations, we can also specify to record the stopping criterion and final cost by adding that to `:Stop` of the sub solver's record. Then we can specify, as usual in a tuple, that the `:Subsolver` should record `:Stop` (by default it takes over `:Iteration`)
...
## Writing own [`RecordAction`](@ref)s

Let's look at an example where we want to count the number of cost function evaluations, again just to illustrate, since for the gradient this is just one evaluation per iteration.
...
function (c::MyCost)(M, p)
    c.count += 1
    return sum(1 / (2 * length(c.data)) * distance.(Ref(M), Ref(p), c.data) .^ 2)
end
```

and we define an own, new [`RecordAction`](@ref), which is a functor, that is a struct that is also a function. The function we have to implement is similar to a single solver step in signature, since it might get called every iteration:
...
function (r::RecordCount)(amp::AbstractManoptProblem, ::AbstractManoptSolverState, k)
    return if k > 0
        push!(r.recorded_values, Manopt.get_cost_function(get_objective(amp)).count)
...
Note that this also illustrates the last use case since you can pass symbol-action pairs into the `record=` array.
...
Now for the plain gradient descent, we have to modify the step to a constant step size.
...
R3 = gradient_descent(
    ...
    record = [
        :Iteration => [
            :Iteration, RecordCount() => :Count, :Cost,
        ],
    ],
...
this is equivalent to calling `R3[:Iteration]`.
...
If we use this counting cost and run the default gradient descent with Armijo line search, we can infer how many Armijo line search backtracks are performed:
