...
for example with a [functor](), as we used in an example in [How to Record Data](https://manoptjl.org/stable/tutorials/HowtoRecord.html)
...
```{julia}
gradient_descent(M, f, grad_f, data[1];
    count=[:Cost, :Gradient],
...
Since caching requires a bit more work, especially when settping up the cache,
