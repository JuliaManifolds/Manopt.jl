...
This tutorial is a short introduction to using solvers for constraint optimization in [`Manopt.jl`](../index.md).

## Introduction

A constraint optimization problem is given by
...
As a second solver, we have the ``[Exact Penalty Method](@ref exact_penalty_method)``{=commonmark}, which currently is available with two smoothing variants, which make an inner solver for smooth optimization, that is by default again [quasi Newton] possible:
