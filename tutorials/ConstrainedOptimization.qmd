...
This can be seen as a balance between moving constraints into the geometry of a manifold $\mathcal M$ and keeping some, since they can be handled well in algorithms, see ``[Bergmann, Herzog, SIAM J. Optim., 2019](@cite BergmannHerzog:2019), [Liu, Boumal, 2019, Appl. Math. Optim](@cite LiuBoumal:2019)``{=commonmark} for details.
...
We will consider the problem of a Nonnegative PCA, cf. Section 5.1.2 in ``[Liu, Boumal, 2019, Appl. Math. Optim](@cite LiuBoumal:2019)``{=commonmark}:
