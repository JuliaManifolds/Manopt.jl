...
A ``[`Hyperrectangle`](@extref `Manifolds.Hyperrectangle`)``{=commonmark} is, in general, not a manifold but a manifold with corners because locally at the boundary it looks like $\mathbb{R}^{n-k} \times \mathbb{R}^k_{\geq 0}$ for some $k > 0$, instead of $\mathbb{R}^n$ as required by the definition of a manifold.

Such spaces require special handling when used as domains in optimization.
For simple methods like gradient descent, using the projected gradient and a stopping criterion involving [`StopWhenProjectedNegativeGradientNormLess`](@ref) may be sufficient; however, methods that approximate the Hessian can benefit from a more advanced approach.
The core idea is considering a piecewise quadratic approximation of the objective along the descent direction in the tangent space at the current iterate, and selecting the generalized Cauchy point -- its minimizer.
...
Then, we can perform standard line search from the initial iterate in the generalized Cauchy direction.

Currently `Manopt.jl` can handle domains that are either a ``[`Hyperrectangle`](@extref `Manifolds.Hyperrectangle`)``{=commonmark} or a ``[`ProductManifold`](@extref `ManifoldsBase.ProductManifold`)``{=commonmark} that has a ``[`Hyperrectangle`](@extref `Manifolds.Hyperrectangle`)``{=commonmark} among its factors, in any position, with other manifolds as the remaining factors.

## Example

Consider the problem of fitting a covariance matrix with box constraints on the variances in principal directions.
The objective is the negative log-probability of the data under a multivariate normal distribution with zero mean and covariance matrix given by the variable to optimize.
...
p0 = ArrayPartition(fill(10.0, N), Matrix{Float64}(I(N)))
p_mle = quasi_Newton(M, logprob_cost, logprob_gradient, p0; stopping_criterion = StopAfterIteration(100) | StopWhenProjectedNegativeGradientNormLess(1.0e-6))
