...
of [Manopt.jl](../index.md). We reuse the optimization tasks from the
...
for example with a [function like object](https://docs.julialang.org/en/v1/manual/methods/#Function-like-objects), as we used in an example in [How to Record Data](HowToRecord.md)
...
function (c::MyCost)(M, p)
    c.count += 1
    return # [ .. Actual implementation of the cost here ]
end
```
...
The approach in the solvers of [Manopt.jl](../index.md) aims to simplify adding
both these capabilities on the level of calling a solver.

## Technical background

The two ingredients for a solver in [Manopt.jl](../index.md)
...
This pattern is also available for the objective, which can be decorated as well.
This is how both caching and counting are implemented, as decorators of the [`AbstractManifoldObjective`](@ref)
and hence for example changing/extending the behavior of a call to [`get_cost`](@ref).

Let's finish off the technical background by loading the necessary packages.
Besides [Manopt.jl](../index.md) and ``[Manifolds.jl](@extref Manifolds :std:doc:`index`)``{=commonmark} we also need
[LRUCache.jl](https://github.com/JuliaCollections/LRUCache.jl) which is a weak dependency and provides
...
```{julia}
#| output: false
using Manopt, Manifolds, Random, LRUCache, LinearAlgebra, ManifoldDiff, BenchmarkTools
...
want to count. A full list is available in the documentation of the
[`ManifoldCountObjective`](@ref).
...
Note that the [automatic differentiation tutorial](AutomaticDifferentiation.md) uses the same Rayleigh quotient with the opposite sign, minimizing toward the smallest eigenvalue rather than maximizing toward the largest.

The Euclidean gradient (that is in $ℝ^{m+1}$) is actually just $\nabla g(p) = -2Ap$, the Riemannian gradient is the projection of $\nabla g(p)$ onto the tangent space $T_p\mathbb S^{m}$.
...
The small disadvantage might be that we always compute _both_ the gradient and the cost. Luckily, the cache we used before takes this into account and caches both results, such that we indeed end up computing `A*p` only once when asking for the cost and the gradient.

Let's compare both methods

```{julia}
p0 = [(1 / sqrt(5) .* ones(5))..., zeros(m - 4)...];
s1 = @btime gradient_descent(
    $N, $g, $grad_g!, $p0;
    stopping_criterion = StopWhenGradientNormLess(1.0e-5),
...
and we can see that the combined number of evaluations is once 1847, once
just the number of cost evaluations 1247. Note that the involved additional 600
...
And while we again have a split of the cost and gradient evaluations, we can observe that this approach allocates only about a quarter of the memory of the previous one.
...
maintaining the whole cache instead of storing just `Ap` and `p` is a bit more costly.
...
While the first approach of [`ManifoldCostGradientObjective`](@ref) is very easy to implement, both the storage and the (local) cache approach are more efficient.
They both improve on the first implementation without sharing interim results, while the combined cost and gradient function allocates more memory than it, since it always computes both.
