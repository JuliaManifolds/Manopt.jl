...
1. An easy approach from within `Manopt.jl`: the [`ManifoldCombinedFirstOrderObjective`](@ref)
...
### The [`ManifoldCombinedFirstOrderObjective`](@ref) approach

The [`ManifoldCombinedFirstOrderObjective`](@ref) uses a combined function to compute both the gradient and the cost at the same time. We define the in-place variant as
...
```{julia}
obj = ManifoldCombinedFirstOrderObjective(g_grad_g!; evaluation=InplaceEvaluation())
...
While the second approach of [`ManifoldCombinedFirstOrderObjective`](@ref) is very easy to implement, both the storage and the (local) cache approach are more efficient.
