...
Besides caching single calls to elements of the objective.
A specific scenario might be, that you want to cache intermediate results
of computing the cost and share that with the gradient. We will present
...
The Rayleigh quotient. We aim to maximize the Rayleigh quotient $\frac{x^{\mathrm{T}}Ax}{x^{\mathrm{T}x}}$, for some $A\in\mathbb R^{m+1\times m+1}$ and $x\in\mathbb R^{m+1}$ but since we consider this on the sphere and `Manopt.jl`
(as many other optimization toolboxes) minimizes, we consider

```math
g(p) = -p^{\mathrm{T}}Ap,\qqad \mathbb S^{m}
```

The Euclidean gradient (that is in $ R^{m+1}$) is actually just $\nabla g(p) = -2Ap$, the Riemannian gradient the projection of $\nablda g(p)$ onto the tangent space $T_p\mathbb S^{m}$.
...
So compared to the previous approach, which might be beneficial.
An alternative is the usage of a functor, that introduces a “shared storage”
of the result of computing `A*p`. We additionally have to store `p` though,
since we have to check that we are still evaluation the cost and/or gradient
at the same point, respectively.
...
function (g::StorageG)(::Val{:Cost}, M, p)
...
function (g::StorageG)(::Val{:Gradient}, M, X, p)
    if !(p==g.p) #We are at a new point -> Update
        g.Ap .= g.A*p
        g.p .= p
    end
    copyto!(M, X, p, -2.0 .* g.Ap)
    project!(M, X, p, X)
    return X
end
```

Here we use a first parameter do distinguish both functions. For the mutating
case the signatures are even different, but for an allocating case, the signatures of the cost and the gradient function are the same.
...
And while we again have a split of the cost and gradient evaluations, we can observer that the allocations are less thatn half of the previous approach.
...
and it is possivle to store more than just the last `p` either cost or gradient
was called with.
...
the last this additional cache does not improve the result further,
it is one the same effort both time and allocation-wise.
