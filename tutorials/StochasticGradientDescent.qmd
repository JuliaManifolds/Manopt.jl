...
solver and different [`DirectionUpdateRule`](@ref)s to introduce
...
```math
\operatorname*{arg\,min}_{p∈\mathcal M} \frac{1}{2N}\sum_{i=1}^{N}
  \operatorname{d}^2_{\mathcal M}(p,p_i),
```

which of course can be (and is) solved by a gradient descent, see the introductory
tutorial or ``[Statistics in Manifolds.jl](@extref Manifolds :std:doc:`features/statistics`)``{=commonmark}.
...
gradf = [(M, p) -> grad_distance(M, q, p) for q in data];
DL = DecreasingLength(M; length = 1.0) # stochastic gradient descent needs a decreasing step size
...
```{julia}
p_opt1 = stochastic_gradient_descent(M, gradF, p0; stepsize = DL)
```
```{julia}
@benchmark stochastic_gradient_descent($M, $gradF, $p0; stepsize = $DL)
```
```{julia}
p_opt2 = stochastic_gradient_descent(M, gradf, p0; stepsize = DL)
```
```{julia}
@benchmark stochastic_gradient_descent($M, $gradf, $p0; stepsize = $DL)
```
...
This result is reasonably close. We can also modify the search direction by using a [`DirectionUpdateRule`](@ref), namely:

On the one hand [`MomentumGradient`](@ref), which keeps track of the iterate and parallel transports the last direction to the current iterate.
...
p_opt3 = stochastic_gradient_descent(
    M, gradf, p0; stepsize = DL, direction = MomentumGradient(; direction = StochasticGradient())
)
```

```{julia}
MG = MomentumGradient(; direction = StochasticGradient());
@benchmark stochastic_gradient_descent($M, $gradf, $p0; stepsize = $DL, direction = $MG)
```

And on the other hand the [`AverageGradient`](@ref) computes an average of the last `n` gradients. This is done by

```{julia}
p_opt4 = stochastic_gradient_descent(
    M, gradf, p0; stepsize = DL,
    direction = AverageGradient(; n = 10, direction = StochasticGradient()),
)
```
```{julia}
AG = AverageGradient(; n = 10, direction = StochasticGradient());
@benchmark stochastic_gradient_descent($M, $gradf, $p0; stepsize = $DL, direction = $AG)
```

Note that the default [`StoppingCriterion`](@ref) here is [`StopAfterIteration`](@ref)`(10000)` combined with [`StopWhenGradientNormLess`](@ref)`(1e-9)`.

For both update rules we have to internally specify that we are still in the stochastic setting,
since both rules can also be used with the ``[`IdentityUpdateRule`](@ref Manopt.IdentityUpdateRule)``{=commonmark} within [`gradient_descent`](@ref).

For this not-that-large-scale example we can of course also use a gradient descent with [`ArmijoLinesearch`](@ref),
