...
The stopping criterion is a function `(problem, state, k) -> Bool` that after the initialization and
...
The [`StoppingCriterionSet`](@ref) is a common supertype for the two specific cases [`StopWhenAll`](@ref) and [`StopWhenAny`](@ref).
These are mapped to the operators `&` and `|`, respectively, so that stopping criteria can be easily combined.

## Functions related to stopping criteria

Of course the main function to implement is the one of the new data structure
`(sc::StopWhenMyNewCriterion)(problem, state, k)` itself.
