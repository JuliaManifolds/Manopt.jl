...
might require a manifold to fill several default values with reasonable values.
...
Therefore, elements like the [`ArmijoLinesearch`](@ref) do accept to provide only some of the optional and keyword arguments as well as to “skip” the manifold. Then a [`ManifoldDefaultsFactory`](@ref) is used to postpone the constructor call until later, when the manifold and other defaults are “filled in” automatically – since before passing a step size to a [solver state](state.md) we have the manifold available.
