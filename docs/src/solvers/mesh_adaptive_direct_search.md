...
* If you do not provide `max_stepsize=` or `scale_mesh=`, their defaults require the [`injectivity_radius`](@extref `ManifoldsBase.injectivity_radius-Tuple{AbstractManifold}`)`(M)`.
