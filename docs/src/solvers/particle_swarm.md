...
* By default the stopping criterion uses the [`norm`](@extref `LinearAlgebra.norm-Tuple{AbstractManifold, Any, Any}`) as well, to stop when the norm of the swarm velocities is small, but if you implemented `inner`, the norm is provided already.
...
* The [`distance`](@extref `ManifoldsBase.distance-Tuple{AbstractManifold, Any, Any}`)`(M, p, q)` when using [`StopWhenChangeLess`](@ref), the default stopping criterion of the [`ParticleSwarmState`](@ref).
