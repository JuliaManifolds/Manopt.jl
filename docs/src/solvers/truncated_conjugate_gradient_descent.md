...
* the [`inner`](@extref `ManifoldsBase.inner-Tuple{AbstractManifold, Any, Any, Any}`)`(M, p, X, Y)`, which is used in every iteration, and the [`norm`](@extref `LinearAlgebra.norm-Tuple{AbstractManifold, Any, Any}`), to check the reduction of the residual; if you implemented `inner`, the norm is provided already.
