...
* A [`mean`](@extref Manifolds :jl:method:`Statistics.mean-Tuple{AbstractManifold, AbstractVector, AbstractVector, ExtrinsicEstimation}`)`(M, population)` has to be available, for example by loading [`Manifolds.jl`](@extref Manifolds :std:doc:`index`) and its [statistics](@extref Manifolds :std:doc:`features/statistics`) tools
