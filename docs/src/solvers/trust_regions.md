...
The trust region sub problem is then defined as
...
Here ``Δ_k`` is the trust region radius, which is adapted in every iteration, and ``\mathcal H_k`` is
...
* [`inner`](@extref `ManifoldsBase.inner-Tuple{AbstractManifold, Any, Any, Any}`)`(M, p, X, Y)` is used within the algorithm step. By default the stopping criterion uses the [`norm`](@extref `LinearAlgebra.norm-Tuple{AbstractManifold, Any, Any}`) as well, to stop when the norm of the gradient is small, but if you implemented `inner`, the norm is provided already.
* if you do not provide both a `max_trust_region_radius=` and a `sub_stopping_criterion=`, a [`manifold_dimension`](@extref `ManifoldsBase.manifold_dimension-Tuple{AbstractManifold}`) is required, since the defaults of both are computed from it.
