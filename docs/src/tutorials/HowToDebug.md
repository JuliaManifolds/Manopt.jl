...
Certain symbols starting with a capital letter are mapped to certain prints, for example `:Cost` is mapped to [`DebugCost`](@ref)`()` to print the current cost function value. A full list is provided in the [`DebugActionFactory`](@ref Manopt.DebugActionFactory(::Symbol)).
...
While in the last step, we specified what to print, this can be extend to even specify *when* to print it. Currently the following four “places” are available, ordered by when they appear
in an algorithm run.
...
- `:Iteration` to print something *after* an iteration. For example the group of prints from
  the last codeblock `[:Iteration, :Cost, " | ", :ϵ, 25,]` is added to this entry.
- `:Stop` to print something when the algorithm stops. In the example above, the `:Stop` adds the [`DebugStoppingCriterion`](@ref) is added to this place.
...
to be printed *before* the iteration is performed.

Changing this in the above run will not change the output.
...
We want to ise this to specify the format to print `ϵ`.
...
Furthermore, we want to print the iteration number *before* printing the subsolvers steps, so we put this into a `Pair`, but we can leave the remaining ones as single
entries.
Finally we also prefix `:Stop` with `" | "`
We get
