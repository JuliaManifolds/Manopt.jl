...
@testset "Subgradient Plan" begin
    ...
    x = [4.0, 2.0]
    x0 = [5.0, 2.0]
    sgs = SubGradientMethodState(M, x0, StopAfterIteration(200), ConstantStepsize(M))
    @test get_iterate(sgs) == x0
    sgs.∂ = [1.0, 0.0]
    ...
    @testset "Allocating Subgradient" begin
        ...
        @test get_initial_stepsize(p, sgs) == 1.0
        @test get_stepsize(p, sgs, 1) == 1.0
        @test get_last_stepsize(p, sgs, 1) == 1.0
        # Check Fallbacks of Problen
        @test get_cost(p, x) == 0.0
        @test norm(M, x, get_subgradient(p, x)) == 0
        @test_throws MethodError get_gradient(p, sgs.x)
        @test_throws MethodError get_proximal_map(p, 1.0, sgs.x, 1)
