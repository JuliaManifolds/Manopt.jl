@testset "Manopt Gradient Descent" begin
    ...
    x3 = gradient_descent(
        ...
        stepsize = NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, 1e-7, π/2, :direct),
        debug = [:Stop]
    )
    ...
    x4 = gradient_descent(
        ...
        stepsize = NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, 1e-7, π/2, :inverse),
        debug = [:Stop]
    )
    ...
    x5 = gradient_descent(
        ...
        stepsize = NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, 1e-7, π/2, :alternating),
        debug = [:Stop]
    )
    @test isapprox(x, x5; atol = 1e-13) 
    
    @test_logs (:warn, string("The strategy 'indirect' is not defined. The 'direct' strategy is used instead.")) NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, 1e-7, π/2, :indirect)
    @test_throws DomainError NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, 0.0, π/2, :direct)
    @test_throws DomainError NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 2, π/2, π/4, :direct)
    @test_throws DomainError NonmonotoneLinesearch(1.0, ExponentialRetraction(), ParallelTransport(), 0.99, 0.1, 0, π/4, π/2, :direct) 
