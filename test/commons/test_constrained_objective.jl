...
@testset "Constrained Plan" begin
    ...
    @testset "Partial Constructors" begin
        ...
        @test get_hess_equality_constraint(M, co1v, p, X, :) == []
    ...
    @testset "is_feasible & DebugFeasibility" begin
        ...
        @test !(@test_logs (:info,) is_feasible(M, coh, p; error = :info))
        @test !(@test_logs (:warn,) is_feasible(M, coh, p; error = :warn))
    ...
    @testset "Lagrangians" begin
        ...
        @testset "Condensed KKT, Jacobian" begin
            ...
            Wc[Nc, 1] += sum(
                (μ ./ s) .*
                    [inner(Nc[1], qc[Nc, 1], gg[i], Yc[Nc, 1]) for i in 1:length(gg)] .* gg,
            )
            # (3) grad h terms (note the Y_2 component)
            Wc[Nc, 1] += sum(Yc[Nc, 2] .* gh)
            # Second component, just h terms
            Wc[Nc, 2] = [inner(Nc[1], qc[Nc, 1], gh[j], Yc[Nc, 1]) for j in 1:length(gh)]
            W = CKKTVfJ(Nc, qc, Yc)
            W2 = copy(Nc, qc, Yc)
            CKKTVfJ(Nc, W2, qc, Yc)
            @test W2 == W
    ...
    @testset "Cache Objective" begin
        ...
        @test get_grad_inequality_constraint!(M, Yi, cccofa, -p, :) == Xi2 # counts for full
        @test Yi == Xi2
        @test get_grad_inequality_constraint!(M, Yi, cccofa, -p, :) == Xi2 # cached
        @test Yi == Xi2
