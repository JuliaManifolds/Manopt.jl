...
mutable struct AdaptiveRegularizationState{
    P,St<:AbstractManoptSolverState,T,R,TStop<:StoppingCriterion,TRTM<:AbstractRetractionMethod          #,
...
function AdaptiveRegularizationState(
    ...
    return AdaptiveRegularizationState{P,St,T,R}(M, p, substate, X, H, S, ς, ρ, ρ_regularization, stop, retraction_method, ςmin , θ, η1, η2, γ1, γ2, γ3)
end
...
function adaptive_regularization!(
    ...
    arcs=AdaptiveRegularizationState(M,
    p;
    substate=substate,
    X=X,
    H=H,
...
mutable struct LanczosState{R}<:AbstractManoptSolverState
    maxIterNewton::R
    tolNewton::R
    Q #type? #Vector{typeof(zero_vector(M,rand(M)))}?
...
function LanczosState(M::AbstractManifold,
...
function subproblem_solver!(dmp::AbstractManoptProblem,s::AdaptiveRegularizationState,ls::LanczosState)
    M = get_manifold(dmp)
