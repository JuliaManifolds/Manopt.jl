...
* `sub_hess` – (a fininte difference approximation by default) specify a Hessian of the subproblem,
   which the default solver, see `sub_state` needs
* `sub_kwargs` (`[]`) pass keyword arguments to the `sub_state`, in form of a `Dict(:kwname=>value)`,
  unless you set the `sub_state` directly.
* `sub_objective` (a [`Grado gradient or hessian objetive based on the last 3 keywords)
...
* `sub_state` ([`TrustRegionState`](@ref) by default, requires `sub_hessian` to be provided; decorated with `sub_kwargs`)
