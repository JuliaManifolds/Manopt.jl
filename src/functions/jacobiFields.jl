...
"""
function adjoint_Jacobi_field(M::Manifold, p, q, t, X, β::Base.Callable=βdifferential_geodesic_startpoint)
...
function adjoint_Jacobi_field(M::Circle{ℝ}, p::Real, q::Real, t::Real, X::Real, β::Base.Callable=βdifferential_geodesic_startpoint)
    x = shortest_geodesic(M, p, q, t)
    B = get_basis(M, p, DiagonalizingOrthonormalBasis(log(M,p,q)))
    V = get_vectors(M, p, B)
    Θ = vector_transport_to.(Ref(M),Ref(p), V, Ref(x), Ref(ParallelTransport()))
...
function adjoint_Jacobi_field(M::AbstractPowerManifold, p, q, t, X, β::Base.Callable=βdifferential_geodesic_startpoint)
...
"""
function jacobi_field(M::Manifold, p, q, t, X, β::Base.Callable=βdifferential_geodesic_startpoint)
    x = shortest_geodesic(M, p, q, t);
    B = get_basis(M, p, DiagonalizingOrthonormalBasis(log(M,p,q)))
    V = get_vectors(M, p, B)
    Θ = vector_transport_to.( Ref(M), Ref(p), V, Ref(x), Ref(ParallelTransport()))
...
function jacobi_field(M::Circle{ℝ}, p::Real, q::Real, t::Real, X::Real, β::Base.Callable=βdifferential_geodesic_startpoint)
    x = shortest_geodesic(M, p, q, t);
    B = get_basis(M, p, DiagonalizingOrthonormalBasis(log(M,p,q)))
    V = get_vectors(M, p, B)
    Θ = vector_transport_to.( Ref(M), Ref(p), V, Ref(x), Ref(ParallelTransport()))
...
function jacobi_field(M::AbstractPowerManifold, p, q, t, X, β::Base.Callable=βdifferential_geodesic_startpoint)
    rep_size = representation_size(M.manifold)
    Y = allocate_result(M, adjoint_Jacobi_field, p, X)
    for i in get_iterator(M)
