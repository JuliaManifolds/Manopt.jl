...
computes ``D_p \gamma(t;p,q)[η]`` (in place of `Y`).
...
computes ``D_q \gamma(t;p,q)[X]`` (in place of `Y`).
...
Note that ``Y ∈  T_X(T_p\mathcal M) = T_p\mathcal M`` is still a tangent vector.
...
@doc raw"""
    differential_geodesic_argument(M, p, X, t, Y)
    differential_geodesic_argument!(M, Z, p, X, t, Y)

computes ``D_X\gamma_{p,X}(t)[Y]`` (in place of `Z`).
Note that ``Y ∈  T_X(T_[\p\mathcal M) = T_p\mathcal M`` is still a tangent vector.
"""
function differential_geodesic_argument(M::AbstractManifold, p, X, t, Y)
    return jacobi_field(M, p, exp(M, p, X), t, Y, βdifferential_exp_argument)
end
function differential_geodesic_argument!(M::AbstractManifold, Z, p, X, t, Y)
    return jacobi_field!(M, Z, p, exp(M, p, X), t, Y, βdifferential_exp_argument)
end

@doc raw"""
    differential_geodesic_startpoint(M, p, X, t, Y)
    differential_geodesic_startpoint!(M, Z, p, X, t, Y)

computes ``D_p\gamma_{p,X}(t)[Y]`` (in place of `Z`).
"""
function differential_geodesic_startpoint(M::AbstractManifold, p, X, t, Y)
    return differential_shortest_geodesic_startpoint(M, p, exp(M, p, X), t, Y)
end
function differential_geodesic_argument!(M::AbstractManifold, Z, p, X, t, Y)
    return differential_shortest_geodesic_startpoint!(M, Z, p, exp(M, p, X), t, Y)
end
