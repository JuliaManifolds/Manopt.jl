...
f(x) = (inner(M,x0,log(M,x0,x),ξ1) - η[2])^2 + (1/ell*(inner(M,x0,log(M,x0,x),ξ2) - η[3]))^2
contour(pts=91,r=0.3) = [ [0., r*cos(α) + η[2], r*ell*sin(α)  + η[3]] for α in range(0,2*π,length=pts) ]
...
stepVec = Tuple(3/4 * η,x0)
stepCurve = shortest_geodesic(M,x0,xHat,[range(0,3/4,length=50)...])
#
...
xP2 = exp(M,xP1,ηP2)
stepCurveP1 = shortest_geodesic(M,xP2,xP1,[range(0,1,length=50)...])
stepCurveP2 = shortest_geodesic(M,xP1,x0,[range(0,1,length=50)...])
