...
function (ipls::InteriorPointLinesearch)(
    ...
    N = get_manifold(mp) × ℝ^length(ams.μ) × ℝ^length(ams.λ) × ℝ^length(ams.s)
    q = allocate_result(N, rand)
    copyto!(N[1], q[N,1], get_iterate(ams))
    copyto!(N[2], q[N,2], ams.μ)
    copyto!(N[3], q[N,3], ams.λ)
    copyto!(N[4], q[N,4], ams.s)
    ...
    (ipls.last_stepsize, ipls.message) = linesearch_backtrack!(
        ...
        q,
        X,
        ipls.initial_guess(mp, ams, i, ipls.last_stepsize),
