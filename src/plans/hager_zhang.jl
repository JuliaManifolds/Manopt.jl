...
function HagerZhangLinesearch(
    ...
    return HagerZhangLinesearch{T,TRM,P}(
        delta,
        sigma,
        alphamax,
        rho,
...
function (ls::HagerZhangLinesearch)(ϕ, ϕdϕ, c::T, phi_0::Real, dphi_0::Real) where {T} # Should c and phi_0 be same type?
    # @unpack delta, sigma, alphamax, rho, epsilon, gamma, linesearchmax, psi3, mayterminate =
    #     ls
    ...
    while iter < linesearchmax
        ...
        iswolfe, iA, iB = secant2!(
            ϕdϕ, alphas, values, slopes, ia, ib, ls.phi_lim, ls.delta, ls.sigma
        )
        if iswolfe
            mayterminate[] = false # reset in case another initial guess is used next
