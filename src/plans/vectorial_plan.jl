...
is ``f: $(_math(:Manifold))nifold))) → ℝ^m`` and we have a basis ``$(_tex(:Cal, "B")) of ``T_p$(_math(:Manifold)))``, at ``p∈ $(_math(:Manifold)))``
...
for example ``g_i(p) ∈ ℝ^m`` or ``$(_tex(:grad)) g_i(p) ∈ T_p$(_math(:Manifold)),
    ``i=1,…,m``.
...
There are two interpretations of the Jacobian of a vectorial function ``F: $(_math(:Manifold))) → ℝ^m`` on a manifold.
Both depend on choosing a basis on the tangent space ``$(_math(:TangentSpace)))`` which we denote by
...
for any ``X∈$(_math(:TangentSpace)))`` we have the equality of the differential ``DF(p)[X] = Jc``.
  In other words, the `j`th column of ``J`` is given by ``DF(p)[Y_j]``

2. Given the gradients ``$(_tex(:grad)) F_i(p)`` of the component functions ``F_i: $(_math(:Manifold))) → ℝ``,
