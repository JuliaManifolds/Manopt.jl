...
struct HessianProblem{mT <: Manifold,TCost,TGradient,THessian,TPrecon} <: Problem
    ...
    hessian::THessian
    precon::TPrecon
    HessianProblem(M::mT,cost,grad,hess,pre) where {mT <: Manifold} = new{mT,typeof(cost),typeof(grad),typeof(hess),typeof(pre)}(M,cost,grad,hess,pre)
end
...
[`trust_regions`](@ref)
"""
mutable struct TrustRegionsOptions{TX,TStop<:StoppingCriterion,TΔ,TΔ_bar,TRetr,Tρ_prime,Tρ_reg} <: HessianOptions
    x::TX
    stop::TStop
    Δ::TΔ
    Δ_bar::TΔ_bar
...
* a tangent vector generated by applying the approximated Hessian to the
    tangent vector ξ
"""
function approxHessianFD(M::MT, x, gradFct, ξ; stepsize=2.0^(-14), transport=ParallelTransport()) where {MT <: Manifold}
    norm_xi = norm(M,x,ξ)
