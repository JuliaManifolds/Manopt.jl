...
"""
struct HessianProblem{mT <: Manifold,TCost<:Base.Callable,TGradient<:Base.Callable,THessian<:Union{Base.Callable,Missing},TPrecon<:Base.Callable} <: Problem
    M::mT
    cost::TCost
    gradient::TGradient
    hessian::THessian
    precon::TPrecon
    HessianProblem(M::mT,cost::Base.Callable,grad::Base.Callable,hess::Union{Base.Callable,Missing},pre::Base.Callable) where {mT <: Manifold} = new{mT,typeof(cost),typeof(grad),typeof(hess),typeof(pre)}(M,cost,grad,hess,pre)
end
...
* a tangent vector generated by applying the approximated Hessian to the
    tangent vector ξ
"""
function approxHessianFD(M::MT, x, gradFct::Base.Callable, ξ; stepsize=2.0^(-14), transport=ParallelTransport()) where {MT <: Manifold}
    norm_xi = norm(M,x,ξ)
