...
md""" # Manopt.jl
A Julia variant of the logo from Manopt
"""
...
grad_f(M, p) = 2*A*p -2*( p'*A*p )*p

# ╔═╡ 41aea70f-6fb4-4e71-b930-89423a78ce08
md"""
## Plotting A Sphere
With the values of ``f`` as colormap
"""
...
# ╔═╡ 4fbd6633-f275-4d0e-ab70-9d1ea86e9207
begin
n = 90
...
for i in 1:n
    for j in 1:n
        ...
        sz[i,j] = cos(v[j]);
		sc[i,j] = f(M, [sx[i,j],sy[i,j],sz[i,j]])
    end
end
π1(x) = 1.0*x[1]
π2(x) = 1.0*x[2]
...
range_sc = [min(sc...), max(sc...)]

# ╔═╡ 04d07f8f-f89a-4c26-b26a-0ac12616f603
begin
	# Colors Manopt, interpolate between
    #     sphere_color = [223 186 105]/255;
    #    plane_color = [255 254 223]/255;
...
cam3d!(lscene)
wireframe!(lscene, sx, sy, sz, color = RGBA(0.5,0.5,0.5,0.3); transparency=true, alpha=0.8)
...
surface!(lscene, sx, sy, sz;
	...
	arrows!(lscene, π1.(vec_base), π2.(vec_base), π3.(vec_base),π1.(vec_point), π2.(vec_point), π3.(vec_point);
...
git-tree-sha1 = "27fc91d7cc346d847c0379c99b1220fcdf5f9f7e"
...
[[deps.ManifoldDiffEq]]
deps = ["Accessors", "ConstructionBase", "DiffEqBase", "LinearAlgebra", "Manifolds", "ManifoldsBase", "Markdown", "OrdinaryDiffEqCore", "RecursiveArrayTools", "SciMLBase", "SimpleUnPack"]
...
git-tree-sha1 = "3b623213064a8bdecfefddfa2169b289ed644e26"
...
git-tree-sha1 = "3754744bb29fe79092338f44e6564eb4b78349ca"
...
git-tree-sha1 = "94c9e63f57f4610fa481ca3f96caed4b03c51e86"
