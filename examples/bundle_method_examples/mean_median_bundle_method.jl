...
@time b_mean = bundle_method(
    M, F, gradF, data[1]; stopping_criterion=StopWhenAny(StopWhenChangeLess(1e-12), StopAfterIteration(5000))
)
@time m_mean = mean(M, data)
mean_dist = distance(M, b_mean, m_mean)
println("Distance between means: $mean_dist")
println("$(F(M, b_mean) <= F(M,m_mean) ? "F(bundle_mean) ≤ F(manifolds_mean)" : "F(bundle_mean) > F(manifolds_mean)")")
...
@time b_median = bundle_method(
    M, F2, gradF2, data[1]; stopping_criterion=StopWhenAny(StopWhenChangeLess(1e-12), StopAfterIteration(5000))
)
@time m_median = median(M, data)
median_dist = distance(M, b_median, m_median)
println("Distance between medians: $median_dist")
println("$(F2(M, b_median) <= F2(M,m_median) ? "F2(bundle_median) ≤ F2(manifolds_median)" : "F2(bundle_median) > F2(manifolds_median)")")
