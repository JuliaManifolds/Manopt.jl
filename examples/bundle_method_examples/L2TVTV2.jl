using ColorSchemes, Random, QuadraticModels, RipQP, Manifolds, Manopt
...
f(N, q) = costL2TVTV2(N, img2, α, β, q)
gradf(N, q) = grad_distance(N, img2, q) + α * grad_TV(N, q) + β * grad_TV2(N, q)
...
asymptote_export_SPD(
    "initial.fig"; data=img, scale_axes=(4.0, 4.0, 4.0), color_scheme=ColorSchemes.hsv
)
render_asymptote("initial.fig")
asymptote_export_SPD(
    "noisy.fig"; data=img2, scale_axes=(12.0, 12.0, 12.0), color_scheme=ColorSchemes.hsv
)
render_asymptote("noisy.fig")
# asymptote_export_SPD("run2_bundle_subnorm.fig"; data=b1, scale_axes=(12.,12.,12.), color_scheme=ColorSchemes.hsv)
# render_asymptote("run2_bundle_subnorm.fig")
asymptote_export_SPD(
    "bundle_bundless.fig";
    data=b2,
...
asymptote_export_SPD(
    "subgradient.fig"; data=s, scale_axes=(12.0, 12.0, 12.0), color_scheme=ColorSchemes.hsv
)
