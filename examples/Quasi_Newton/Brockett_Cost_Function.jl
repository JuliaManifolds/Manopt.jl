...
x = random_point(M)
@time quasi_Newton(M,F,∇F,x; memory_size = 4, vector_transport_method = ProjectionTransport(), stopping_criterion = StopWhenGradientNormLess(norm(M,x,∇F(x))*10^(-6)), debug = [:Iteration, " ", :Cost, "\n", 1, :Stop]) 