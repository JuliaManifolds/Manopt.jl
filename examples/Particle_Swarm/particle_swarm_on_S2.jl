...
euclidean_to_spherical(x,y,z) = [acos(z/sqrt(x^2+y^2+z^2)), atan(y,x), sqrt(x^2 + y^2 + z^2)]
...
# Init contour plot
scene = Scene()
mesh!(scene, Makie.Sphere(Point3f0(0), 0.9975f0), color=data, colormap = ColorSchemes.viridis, levels=50, interpolate=false, show_axis = false)
#
# Run PSO
# (a) equiangular x0
N = 50
x0 = [ random_point(M) for i=1:200 ]
xPlot = deepcopy(x0)
v0 = [zero_tangent_vector(M,y) for y ∈ x0]
o = particle_swarm(M,F; x0 = x0, velocity = v0, inertia = 0.05, social_weight = 0.3, cognitive_weight = 0.2,
    stopping_criterion = StopAfterIteration(800),
    debug=[:Iteration,"\n",100], record = [:x, :p, :velocity], return_options=true
);
...
record(scene, string(@__DIR__, "/record_video.mp4"), 1:N, framerate=30) do i
    t[] = path[i]
    b[] = path[i][ argmin([F(y) for y in path[i]]) ]
end
